...
export function Sidebar({ user, open = true, mobile = false, onClose }: { user: CurrentUser; open?: boolean; mobile?: boolean; onClose?: () => void }) {
  ...
  return (
    <aside className={cn(mobile ? 'cms-sidebar-mobile h-full min-h-0 w-full' : 'min-h-screen w-64', 'shrink-0 overflow-y-auto border-r border-border bg-panel')} aria-label="Điều hướng CMS">
      <div className="flex items-start justify-between gap-3 border-b border-border px-5 py-5">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">CMS nội bộ</p>
        </div>
      </div>
