...
export function WorkPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-accent">Hàng đợi xử lý</p><h1 className="mt-1 text-2xl font-bold text-text">Việc của tôi</h1><p className="mt-2 text-sm text-text-muted">Ưu tiên công việc theo hạn xử lý, SLA và bước nghiệp vụ tiếp theo.</p></div><Button variant="primary">Thêm ứng viên</Button></div>
      <WorkSummary summary={summary} activeView={params.view} onSelect={setView} />
      <SavedViewBar><div className="flex flex-wrap items-center gap-2"><label htmlFor="work-search" className="text-sm font-semibold text-text">Tìm trong việc</label><input id="work-search" value={params.query} onChange={(event) => setQuery(event.target.value)} placeholder="Tên ứng viên, khách hàng, công việc" className="min-h-10 w-72 rounded-control border border-border bg-panel px-3 text-sm" /></div><label className="flex items-center gap-2 text-sm text-text-muted">View<select value={params.view} onChange={(event) => setView(event.target.value)} className="min-h-10 rounded-control border border-border bg-panel px-3 text-sm"><option value="actionable">Cần xử lý</option><option value="today">Hôm nay</option><option value="overdue">Quá hạn</option><option value="waiting-reply">Chờ phản hồi</option></select></label></SavedViewBar>
